...
import story_renderer_v4


MODE_EXPORTER_VERSION = "mode-exporter-v9.0"
...
def add_preview_sheet(wb: Workbook, package: dict) -> int:
    ...
    ws["A2"] = "このシートの画像は、アプリと同一のレンダラーで生成されています。"
...
def _context_rows(package: dict) -> list[dict]:
    ...
    return [
        ...
        {"item": "renderer", "value": quality.get("renderer") or story_renderer_v4.STORY_RENDERER_VERSION},
        ...
        {"item": "cluster_coherence", "value": hero.get("cluster_coherence")},
        {"item": "event_fingerprint", "value": hero.get("event_fingerprint")},
...
def _candidate_rows(package: dict) -> list[dict]:
    ...
    for c in (package.get("story_context") or {}).get("candidates") or []:
        rows.append({
            ...
            "cluster_size": c.get("cluster_size"), "cluster_coherence": c.get("cluster_coherence"), "event_fingerprint": c.get("event_fingerprint"),
            "entities": c.get("entities"), "entity_details": c.get("entity_details"), "sources": c.get("source_names"),
            "why_now_ja": c.get("why_now_ja"), "conflict_ja": c.get("conflict_ja"), "implication_ja": c.get("implication_ja"),
        })
    return rows
...
def _card_rows(package: dict) -> list[dict]:
    ...
    for set_name, card in _renderable_cards(package):
        ...
        rows.append({
            ...
            "event_ref_score": (card.get("qa") or {}).get("event_ref_score"), "source": card.get("source"),
            "layout": direction.get("layout_variant"), "scene_type": direction.get("scene_type"),
            "visual_prompt_4_5": (direction.get("image_prompts") or {}).get("4:5"),
        })
    return rows
...
def build_story_excel(package: dict, resources: list[dict]) -> bytes:
    ...
    headers = ["id", "headline_ja", "archetype", "topic", "story_score", "hero_story_score", "confidence", "cluster_size", "cluster_coherence", "event_fingerprint", "entities", "entity_details", "sources", "why_now_ja", "conflict_ja", "implication_ja"]
    _append_rows(candidates, headers, _candidate_rows(package))

    cards = wb.create_sheet("Story_Cards")
    headers = ["set", "slide", "story_role", "story_archetype", "headline", "body", "evidence_excerpt", "evidence_refs", "fact_bound", "event_ref_score", "source", "layout", "scene_type", "visual_prompt_4_5"]
