...
import story_renderer_v2


MODE_EXPORTER_VERSION = "mode-exporter-v7.0"
...
def _render_png(package: dict, card: dict, width: int, height: int) -> bytes:
...
def add_preview_sheet(wb: Workbook, package: dict) -> int:
    ...
    ws["A2"] = "このシートの画像は、アプリのプレビューと同じレンダラーで生成されています。"
    ...
    for index, (set_name, card) in enumerate(_renderable_cards(package)):
        ...
        ws[f"{col}{row_cursor + 1}"] = str(card.get("story_role") or card.get("card_type") or "")
        stream = BytesIO(_render_png(package, card, width=540, height=675))
...
def _story_context_rows(package: dict) -> list[dict]:
    ...
    facts = context.get("evidence_facts") or {}
    return [
        {"item": "pipeline", "value": (package.get("content_quality") or {}).get("pipeline")},
        {"item": "engine", "value": (package.get("content_quality") or {}).get("engine")},
        ...
        {"item": "evidence_facts", "value": facts},
        {"item": "generation_seed", "value": (package.get("content_quality") or {}).get("generation_seed")},
        {"item": "renderer", "value": story_renderer_v2.STORY_RENDERER_VERSION},
    ]
        ...
        rows.append(
            {
                ...
                "evidence_excerpt": card.get("evidence_excerpt"),
...
def build_story_excel(package: dict, resources: list[dict]) -> bytes:
    ...
    _append_rows(
        cards,
        ["set", "slide", "story_role", "story_archetype", "headline", "body", "evidence_excerpt", "evidence_refs", "source", "layout", "visual_prompt_4_5"],
