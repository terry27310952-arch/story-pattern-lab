...
# Lighten the metric cards and overall top section to stand out against Streamlit's dark or black backgrounds.
st.markdown(
    """
    <style>
    /* Override the default metric container to use a light background and clear borders */
    ...
    /* Label of metric (the top line) */
    div[data-testid="stMetric"] > label {
        color: #333333 !important;
        font-weight: 600 !important;
    }
    /* Value of metric (the number) */
    div[data-testid="stMetric"] > div {
        color: #005DAA !important;
        font-size: 26px !important;
        font-weight: 700 !important;
    }
    /* Ensure metric rows don't collapse margins */
...
with tabs[2]:
    ...
    if not rows:
        ...
        # Show raw counts of likes, comments and views for the selected story
        count_cols = st.columns([1, 1, 1])
        with count_cols[0]:
            like_cnt = selected.get('like_count')
            st.metric("좋아요", like_cnt if like_cnt else "N/A")
        with count_cols[1]:
            comment_cnt = selected.get('comment_count')
            st.metric("댓글수", comment_cnt if comment_cnt else "N/A")
        with count_cols[2]:
            view_cnt = selected.get('view_count')
            st.metric("조회수", view_cnt if view_cnt else "N/A")
