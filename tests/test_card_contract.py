...
class CardContractTest(unittest.TestCase):
    ...
    def test_generates_japanese_cards_with_locked_brand_outro(self) -> None:
        ...
        self.assertIn("勢力ハンター", visible_card_text(self.cards[-1]))
        self.assertIn("キヨサキ", visible_card_text(self.cards[-1]))
        self.assertIn("勢力が入ったポイントを無料でチェック", visible_card_text(self.cards[-1]))
    ...
    def test_external_backend_failure_falls_back_without_crash(self) -> None:
        ...
        self.assertEqual(len(package["cards"]["6장"]), 7)
        self.assertEqual(package["cards"]["6장"][-1]["card_type"], BRAND_OUTRO_TYPE)
